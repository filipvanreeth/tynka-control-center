...
- ~~`\PDO` in de query-handlers~~ **opgelost (Fase C):** de leeskant loopt nu via de port
  ...
  (`CheckIn/Infrastructure/Persistence/`). `\PDO` zit enkel nog in de twee Persistence-adapters
  en in `index.php`. De controller injecteert `GetTopHandlersHandler` (niet meer inline gebouwd).
- Domain Events & Command-handlers nog niet in gebruik (de `AggregateRoot`-basisklasse
  staat er wel; events recorden = latere stap).
- Bedrading gebeurt handmatig met `new` in `index.php`; PHP-DI zit in composer maar
  is nog niet als container in gebruik.
- `edit()`-route is geparkeerd/kapot: `GetCheckInHandler` bouwt `CheckInData` met een
  verouderde signatuur (mist `HandlerData` + opties) en gebruikt nog `CheckIn->uuid()`;
  `edit()` roept `getAllCheckInOptionsHandler->handle()` zonder `$query` aan. Fixen zodra
  de rij→DTO-mapping gecentraliseerd is (`CheckInData::fromDomain()`).
